...
import pandas as pd
from utils2 import stddev, rank, ts_argmax, correlation, covariance, delta, ts_rank, sma, ts_sum, delay, ts_max, ts_min, scale
...
class Alphas(object):
    ...
    def alpha017(self):
        ...
        return -1 * (rank(ts_rank(self.close, 10)) * rank(
            delta(delta(self.close, 1), 1)) * rank(
                ts_rank((self.volume / adv20), 5)))
    ...
    def alpha024(self):
        cond = delta(sma(self.close, 100), 100) / delay(self.close,
                                                        100) <= 0.05
    ...
    def alpha031(self):
        ...
        return ((rank(
            rank(
                rank(
                    decay_linear((-1 * rank(rank(delta(
                        self.close, 10)))), 10)))) + rank(
                            (-1 * delta(self.close, 3)))) + sign(scale(df)))
    ...
    def alpha045(self):
        ...
        df = df.replace([-np.inf, np.inf], 0).fillna(value=0)
        return -1 * (rank(sma(delay(self.close, 5), 20)) * df * rank(
    ...
    def alpha052(self):
        return (((-1 * delta(ts_min(self.low, 5), 5)) * rank(
    ...
    def alpha054(self):
        ...
        return -1 * (self.low - self.close) * (self.open**5) / (inner * (
            self.close**5))
    ...
    def alpha101(self):
        return (self.close - self.open) / ((self.high - self.low) + 1e-3)
